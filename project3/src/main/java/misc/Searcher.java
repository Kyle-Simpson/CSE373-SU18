...
public class Searcher {
    ...
    public static <T extends Comparable<T>> IList<T> topKSort(int k, IList<T> input) {
        ...
        // set the condition to see how the K works
        if (k<0) {
        ...
        // when the k > 0, we need add the items to the list; then, we check the size if it is
        // smaller than the K (we need to keep the k as our biggest number)
        // it runs O(n); and the condition gives the log(k) when the size is bigger, we compare the minimum
        // and insert the item. So, the result is always biggest.
        // the method give O(n*log(k))
        for (T item: input) {
            if (heap.size()<k) {
                heap.insert(item);
            }else {
