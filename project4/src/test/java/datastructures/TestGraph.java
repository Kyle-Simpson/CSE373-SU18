...
public class TestGraph extends BaseTest {
    ...
    public Graph<String, SimpleEdge<String>> buildLectureGraph() {
        IList<String> vertices = new DoubleLinkedList<>();
        vertices.add("a");
        vertices.add("b");
        vertices.add("c");
        ...
        edges.add(edge("a", "f", 1));
        
        edges.add(edge("b", "c", 2));
        edges.add(edge("b", "g", 2));
        edges.add(edge("b", "f", 5));
        edges.add(edge("c", "d", 1));
        edges.add(edge("c", "g", 1));
        edges.add(edge("c", "h", 3));
        edges.add(edge("d", "e", 6));
        edges.add(edge("d", "h", 3));

        edges.add(edge("e", "h", 2));

        edges.add(edge("f", "g", 2));
        edges.add(edge("g", "h", 4));
    ...
    @Test(timeout=SECOND)
    public void testFindingMstFromLectureGraph() {
        Graph<String, SimpleEdge<String>> graph = this.buildLectureGraph();
        ISet<SimpleEdge<String>> mst = graph.findMinimumSpanningTree();

        assertEquals(graph.numVertices() - 1, mst.size());
        
        assertTrue(mst.contains(edge("a", "f", 1)));
        assertTrue(mst.contains(edge("f", "g", 2)));
        assertTrue(mst.contains(edge("c", "g", 1)));
        assertTrue(mst.contains(edge("b", "c", 2)));
        assertTrue(mst.contains(edge("c", "d", 1)));
        assertTrue(mst.contains(edge("e", "h", 2)));
        assertTrue(mst.contains(edge("d", "h", 3)) || mst.contains(edge("d", "h", 3)));
    }
